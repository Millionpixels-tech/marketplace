...
export const generateCustomOrderAcceptanceSellerEmail = (customOrder: CustomOrder, orders: Order[]) => {
    const subject = `Custom Order Accepted - ${customOrder.items.length} Item${customOrder.items.length > 1 ? 's' : ''} from ${customOrder.buyerName}`;
    ...
    const html = `
        ...
        <head>
            ...
            <title>Custom Order Accepted - ${FROM_NAME}</title>
            <!--[if mso]>
            <noscript>
                <xml>
            ...
            <style type="text/css">
                ...
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                @media only screen and (max-width: 640px) {
                    ...
                    .action-title { font-size: 20px !important; }
                    .action-icon { font-size: 40px !important; }
                    ...
                    <td align="center" style="padding: 0;">
                        ...
                        <div style="max-width: 640px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);">
                            ...
                            <div style="padding: 32px 24px;">
                                ...
                                <!-- Action Banner -->
                                <div style="background: linear-gradient(135deg, #e8f5e8 0%, #d1f2d1 100%); border: 2px solid #72b01d; border-radius: 10px; padding: 24px; margin-bottom: 32px; text-align: center;">
                                    <h2 style="color: #155724; font-size: 22px; font-weight: 600; margin: 0 0 8px 0; line-height: 1.3;">
                                        Custom Order Accepted Successfully!
                                    </h2>
                                    <p style="color: #155724; font-size: 16px; margin: 0; line-height: 1.5;">
                                        You have accepted the custom order from <strong>${customOrder.buyerName}</strong>. ${orders.length} order${orders.length > 1 ? 's have' : ' has'} been created.
                                    </p>
                                </div>
                                ...
                                <div style="background-color: #fff; border: 1px solid #e2e8f0; border-radius: 10px; margin-bottom: 24px; overflow: hidden;">
                                    ...
                                    <div style="padding: 24px;">
                                        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                            <tr>
                                                <td style="padding: 8px 0; color: #454955; font-size: 15px; width: 40%;">Custom Order ID:</td>
                                                <td style="padding: 8px 0; color: #0d0a0b; font-size: 15px; font-weight: 600;">${customOrder.id}</td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 8px 0; color: #454955; font-size: 15px;">Buyer:</td>
                                                <td style="padding: 8px 0; color: #0d0a0b; font-size: 15px; font-weight: 600;">${customOrder.buyerName}</td>
                                            </tr>
                                            ${customOrder.buyerEmail ? `
                                            <tr>
                                                <td style="padding: 8px 0; color: #454955; font-size: 15px;">Buyer Email:</td>
                                                <td style="padding: 8px 0; color: #0d0a0b; font-size: 15px; font-weight: 600;">${customOrder.buyerEmail}</td>
                                            </tr>
                                            ` : ''}
                                            ${customOrder.buyerPhone ? `
                                            <tr>
                                                <td style="padding: 8px 0; color: #454955; font-size: 15px;">Buyer Phone:</td>
                                                <td style="padding: 8px 0; color: #0d0a0b; font-size: 15px; font-weight: 600;">${customOrder.buyerPhone}</td>
                                            </tr>
                                            ` : ''}
                                            <tr>
                                            ...
                                            <tr>
                                                <td style="padding: 8px 0; color: #454955; font-size: 15px;">Orders Created:</td>
                                                <td style="padding: 8px 0; color: #72b01d; font-size: 15px; font-weight: 600;">${orders.length} order${orders.length > 1 ? 's' : ''}</td>
                                            </tr>
                                        </table>
                                    </div>
                                </div>
                                    ...
                                    <div style="padding: 24px;">
                                        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                            ...
                                            <tr style="border-top: 2px solid #72b01d;">
                                                <td style="padding: 16px 0 8px 0; color: #4a5568; font-size: 15px; font-weight: 600;">Total Earnings:</td>
                                ...
                                <div style="background: linear-gradient(135deg, #fff3cd 0%, #ffeaa7 100%); border: 2px solid #ffc107; border-radius: 10px; padding: 20px; margin: 24px 0;">
                                    ...
                                    <p style="color: #856404; font-size: 14px; line-height: 1.5; margin: 0;">
                                        The custom order has been automatically split into <strong>${orders.length} separate orders</strong> for easier processing and tracking. Each order can be managed independently in your dashboard.
                                    </p>
                                </div>
                                ` : ''}
                                
                                <!-- Next Steps -->
                                <div style="background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%); border: 2px solid #72b01d; border-radius: 10px; padding: 20px; margin: 24px 0;">
                                    <h4 style="color: #1e40af; font-size: 16px; font-weight: 600; margin: 0 0 8px 0;">
                                        🚀 Next Steps
                                    </h4>
                                    <p style="color: #1e40af; font-size: 14px; line-height: 1.5; margin: 0 0 12px 0;">
                                        • Process and prepare the ordered items<br>
                                        • Contact ${customOrder.buyerName} to confirm delivery details<br>
                                        • Update order status as you progress<br>
                                        • Arrange shipping or delivery coordination
                                    </p>
                                    <p style="color: #1e40af; font-size: 13px; margin: 0; font-style: italic;">
                                        The buyer has been notified and is expecting updates on the order progress.
                                    </p>
                                </div>
                                
                                <!-- CTA Button -->
                                <div style="text-align: center; margin: 32px 0;">
                                    <a href="${typeof window !== 'undefined' ? window.location.origin : 'https://sina.lk'}/seller/orders" style="display: inline-block; background: linear-gradient(135deg, #72b01d 0%, #5a8c17 100%); color: #ffffff !important; text-decoration: none; padding: 16px 32px; border-radius: 8px; font-size: 16px; font-weight: 600; text-align: center; border: none; box-shadow: 0 4px 12px rgba(114, 176, 29, 0.3);">
                                        📱 Manage Orders
                                    </a>
                                </div>
                            ...
                            <div style="background-color: #f8fafc; padding: 24px; text-align: center; border-top: 1px solid #e2e8f0;">
                                <p style="color: #454955; font-size: 14px; line-height: 1.5; margin: 0 0 8px 0;">
                                    Need help managing your orders? Contact us at <a href="mailto:seller-support@sina.lk" style="color: #72b01d; text-decoration: none;">seller-support@sina.lk</a>
                                </p>
                                <p style="color: #454955; font-size: 14px; line-height: 1.5; margin: 0 0 8px 0;">
