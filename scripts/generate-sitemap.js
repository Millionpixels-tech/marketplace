...
function generateSitemap() {
  ...
  categories.forEach(category => {
    ...
    urls.push(generateUrlEntry(`/search?category=${encodedCategory}`, '0.8', 'daily'));
  });
  ...
  priceRanges.forEach(({ min, max }) => {
    urls.push(generateUrlEntry(`/search?minPrice=${min}&maxPrice=${max}`, '0.6', 'weekly'));
  });
  ...
  // Add subcategory combinations
  Object.entries(popularSubcategories).forEach(([category, subcategories]) => {
    const encodedCategory = encodeURIComponent(category);
    subcategories.forEach(subcategory => {
      const encodedSubcategory = encodeURIComponent(subcategory);
      urls.push(generateUrlEntry(`/search?category=${encodedCategory}&subcategory=${encodedSubcategory}`, '0.6', 'daily'));
    });
  });
  ...
  categories.slice(0, 5).forEach(category => {
    searchTerms.slice(0, 3).forEach(term => {
      const encodedCategory = encodeURIComponent(category);
      const encodedTerm = encodeURIComponent(term);
      urls.push(generateUrlEntry(`/search?category=${encodedCategory}&q=${encodedTerm}`, '0.6', 'weekly'));
    });
  });
